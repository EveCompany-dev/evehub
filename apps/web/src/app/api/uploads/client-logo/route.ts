...
/** Client brand logo — shown on the client's card/page and on relation cells in Tabelas. */
export async function POST(request: Request): Promise<Response> {
  return handle(async () => {
    ...
    try {
      const saved = await saveUpload(file, 'client-logos', { maxBytes: MAX_BYTES, allowedTypes: ALLOWED_TYPES });
